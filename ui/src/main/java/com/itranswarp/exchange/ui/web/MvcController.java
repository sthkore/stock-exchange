...
public class MvcController extends LoggerSupport {
    ...
    public String depositAmount(@RequestParam String emailAddr){
        UserProfileEntity userProfile=userService.fetchUserProfileByEmail(emailAddr);
        if(userProfile!=null){
            Random random = new Random(System.currentTimeMillis());
            Integer randomAmount= random.nextInt(100,10000);
            BigDecimal randomDecimalAmount=new BigDecimal(randomAmount);
            logger.info("randomAmount:{}",randomDecimalAmount);
            deposit(userProfile.userId, AssetEnum.BTC, randomDecimalAmount);
            deposit(userProfile.userId, AssetEnum.USD, randomDecimalAmount);
        }
        return "ok";
    }
    ...
    private void deposit(Long userId, AssetEnum asset, BigDecimal amount) {
        ...
        // req.transferId = HashUtil.sha256(IdUtil.generateUniqueId());
        req.amount = amount;
        req.asset = asset;
        req.fromUserId = UserType.DEBT.getInternalUserId();
        req.toUserId = userId;
        logger.info("userId:{},transferId:{},asset:{},amount:{}",userId,req.transferId,asset.name(),amount);
